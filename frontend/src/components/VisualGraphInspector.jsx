  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* 2D Topology Visualizer */}
        <div className="lg:col-span-2 glass-card p-6 rounded-2xl border border-amber-500/20 flex flex-col items-center justify-center space-y-6 min-h-[420px]">
          {/* MASTER ORCHESTRATOR NODE */}
          {nodes.filter(n => n.agent_type === 'Orchestrator').map(node => (
            <div
              key={node.node_id}
              onClick={() => onSelectNode(node.node_id)}
              className={`cursor-pointer glass-card p-5 rounded-2xl border transition duration-300 text-center w-72 glow-amber ${
                selectedNodeId === node.node_id ? 'border-amber-400 ring-2 ring-amber-500/50' : 'border-slate-800'
              }`}
            >
              ...
              <div className="text-xs text-amber-300 font-mono">Status: {node.status}</div>
          ...
          <div className="w-0.5 h-8 bg-gradient-to-b from-amber-400 to-slate-700"></div>
            ...
            {nodes.filter(n => n.agent_type === 'Subagent').map(node => (
              <div
                ...
                className={`cursor-pointer glass-card p-4 rounded-xl border transition text-center hover:scale-105 ${
                  selectedNodeId === node.node_id ? 'border-amber-400 ring-2 ring-amber-500/50' : 'border-slate-800'
                } ${node.status === 'RUNNING' ? 'pulse-node border-amber-400' : ''}`}
              >
                <div className="flex items-center justify-center space-x-1.5 mb-1">
                  <span className={`w-2 h-2 rounded-full ${node.status === 'RUNNING' ? 'bg-amber-400' : 'bg-slate-600'}`}></span>
        ...
        <div className="glass-card p-5 rounded-2xl border border-amber-500/20 flex flex-col h-full space-y-3">
          ...
          <div className="flex-1 bg-slate-950 rounded-xl p-3 font-mono text-[11px] text-amber-300 overflow-auto max-h-[380px] border border-slate-800">
            {selectedNode ? (
